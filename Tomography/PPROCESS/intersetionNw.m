...
%% Function to calculate refractivites in the hoorizontal coordinates of the intersection point between ray path and tomography vertical layer
    ...
    % Layer midpoints: one value per layer, matching height = diff(model.levels_TOMO)./1000
    midLevels = 0.5 * (model.levels_TOMO(1:end-1) + model.levels_TOMO(2:end));  % [m], numel = nLayers
    nLayers = numel(midLevels);
    if size(MI,1) > 0
        for j = 1:size(MI,1)
            [lon_ray(j),lat_ray(j),alt_ray(j)] = cspice_recgeo(MI(j,:)'  ,radii(1),(radii(1)-radii(2))/radii(1));
            if  alt_ray > 0
                for z = 1:nLayers
                    N = IDW_atom(model.LAT, model.LON, model.temp{epoch,1}, model.wvpr{epoch,1}, model.pres{epoch,1},model.refr{epoch,1},model.refrRT{epoch,1} ,model.rWGS,[(pi()/2-lat_ray(j)) lon_ray(j), midLevels(z)],model.levels_TOMO_RT.*1000',switches);
                    Nw(j,z) = N.Nw;
                end 

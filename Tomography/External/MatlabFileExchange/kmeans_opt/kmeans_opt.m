...
%     [~,~,dist]=kmeans(X,c,'emptyaction','drop'); %compute the sum of intra-cluster distances
    %     [L,C]=kmeans(X,c); %compute the sum of intra-cluster distances
    %     c = 100;
    [L,C]=kmeans(X,c); %compute the sum of intra-cluster distances
    dist=NaN(c,1);
    for cl = 1:c
        pind = find(L==cl);
        clpcrds = X(:,pind);
        dist(cl,1) = sum(sqrt(sum((clpcrds-C(:,cl)).^2)));
    end
    clear L C pind clpcrds
...
[IDX,C]=kmeans(X,K); %compute the sum of intra-cluster distances
SUMD = NaN(K,1);
        for cl = 1:K
            pind = find(IDX==cl);
            clpcrds = X(:,pind);
            SUMD(cl,1) = sum(sqrt(sum((clpcrds-C(:,cl)).^2)));
        end
        clear pind clpcrds

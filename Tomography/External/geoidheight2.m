...
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    function checklatitude()
        ...
        [latwrapped,lat,lon] = wraplatitude2(lat,lon,'deg');
        if latwrapped
...
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    function checklongitude()
        [lonwrapped, lon] = wraplongitude2(lon,'deg','360');
...
function [lat_wrapped, lat, lon] = wraplatitude2( lat, lon, units)
...
function [lon_wrapped, lon] = wraplongitude2( lon, inputUnits, maxRange )

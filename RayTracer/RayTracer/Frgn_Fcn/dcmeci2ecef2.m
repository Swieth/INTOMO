function varargout = dcmeci2ecef2(reduction,UTC,varargin)
...
%Validate reduction
reduction = lower(p.Results.reduction);
validReduction = {'iau-2000/2006','iau-76/fk5'};
reduction = validatestring(reduction,validReduction);
...
jdTT = mjuliandate2(year,month,day,hour,min,ssTT);
...
switch reduction
    case 'iau-2000/2006'
        ...
        tTT5 = tTT4.*tTT;
        % Julian date for UT1
        jdUT1 = mjuliandate2(year,month,day,hour,min,ssUT);
        ...
        %% Polar motion
        % TIO locator
        sp = convang2(-0.000047*tTT/3600,'deg','rad');
        % Transformation matrix for polar motion
        W = angle2dcm2(sp,-p.Results.polarMotion(:,1),-p.Results.polarMotion(:,2),'ZYX');
        ...
        R = angle2dcm2(thetaERA,zeros(len,1),zeros(len,1));
        ...
        nutationV = mod([convang2([mMoon; mSun; umMoon; dSun; omegaMoon]/3600,'deg','rad'); ...
        ...
        X = convang2(X*1e-6/3600,'deg','rad') + p.Results.dCIP(:,1)';
        Y = convang2(Y*1e-6/3600,'deg','rad') + p.Results.dCIP(:,2)';
        S = convang2(S*1e-6/3600,'deg','rad')-X.*Y/2;
        % Coordinates of the CIP
        E = atan2(Y,X);
        d = atan(sqrt((X.^2+Y.^2)./(1-X.^2-Y.^2)));
        % Transformation matrix for celestial motion of the CIP
        Q = angle2dcm2(E,d,-E-S,'ZYZ');
    ...
    case 'iau-76/fk5'
        ...
        % Additional time calculations:
        jdUT1 = mjuliandate2(year,month,day);
        ...
        epsilonBar = convang2(23.439291 - 0.0130042*tTT - 1.64e-7*tTT2 + 5.04e-7*tTT3,'deg','rad');
        ...
        omegaMoon = convang2(125.04455501 - (5*360 + 134.1361851)*tTT ...
                                    + 0.0020756*tTT2 + 2.139e-6*tTT3,'deg','rad');
        omegaMoon(jdUT1<50449) = 0;
        % Equation of the equinoxes
        equinoxEq = dpsi.*cos(epsilonBar) + convang2(0.00264/3600,'deg','rad')*...
            sin(omegaMoon) + convang2(0.000063/3600,'deg','rad')*sin(2*omegaMoon); 
        % Greenwhich apparent sidereal time
        thGAST = thGMST*pi/180 + equinoxEq;
        % Transformation matrix for earth rotation
        R = angle2dcm2(thGAST,zeros(len,1),zeros(len,1));
        % True obliquity of ecliptic
        epsilon = epsilonBar+depsilon;
        % True equator to mean equinox date transformation matrix
        N = angle2dcm2(epsilonBar,-dpsi,-epsilon,'XZX');
        %% Precession
        % Zeta, theta and z represent the combined effects of general
        % precession.
        zeta = convang2((2306.2181*tTT + 0.30188*tTT2 + 0.017998*tTT3)/3600,'deg','rad'); 
        theta = convang2((2004.3109*tTT - 0.42665*tTT2 - 0.041833*tTT3)/3600,'deg','rad');
        z = convang2((2306.2181*tTT + 1.09468*tTT2 + 0.018203*tTT3)/3600,'deg','rad'); 
        % Mean equinox to celestial reference frame
        P = angle2dcm2(-zeta,theta,-z,'ZYZ');
        %% Polar motion

function [ varargout ] = unitconversion2( mtype, iunit, ounit, useport )
% UNITCONVERSION internal function containing common algorithm for unit
% conversion. 
  
%   Copyright 2000-2016 The MathWorks, Inc.

error(nargoutchk(0,3, nargout,'struct')); %#ok<NCHKE>

if ~ischar( mtype ) && ~isstring( mtype )
    error(message('aero:unitconversion:notChar1'));
end

if ~ischar( iunit ) && ~isstring( iunit )
    error(message('aero:unitconversion:notChar2'));
end

if ~ischar( ounit ) && ~isstring( ounit )
...
ConvertStruct = getunitdata2(mtype,iunit,ounit);

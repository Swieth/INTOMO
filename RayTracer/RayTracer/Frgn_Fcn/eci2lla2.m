function lla = eci2lla2(P,UTC,varargin)
...
%
%   Calculate the latitude, longitude and altitude for the ECI cooordinates
...
switch reduction
    case 'iau-76/fk5'
        dcm = dcmeci2ecef2(p.Results.reduction,p.Results.UTC,p.Results.deltaAT,...
...
lla = ecef2lla2(ecef,p.Results.flattening,p.Results.RE);

function validateReduction(reduction,validReduction)
reduction = lower(reduction);

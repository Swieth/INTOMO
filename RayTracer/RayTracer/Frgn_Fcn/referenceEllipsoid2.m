classdef referenceEllipsoid2 < oblateSpheroid
...
%
%   See also geodetic2ecef, ecef2geodetic, ecefOffset, referenceSphere, validateLengthUnit, wgs84Ellipsoid
...
% Copyright 2011-2017 The MathWorks, Inc.
    ...
    properties (GetAccess = private, Constant = true)
        ...
        Ellipsoids = { ...
            ...
            7035, 'sphere',        'SemimajorAxis', 6371000,     'Eccentricity',       0,             'Spherical Earth'; ...
            7019, 'grs80',         'SemimajorAxis', 6378137,     'InverseFlattening',  298.257222101, 'Geodetic Reference System 1980'; ...
            7030, 'wgs84',         'SemimajorAxis', 6378137,     'InverseFlattening',  298.257223563, 'World Geodetic System 1984'; ...
            7015, 'everest',       'SemimajorAxis', 6377276.345, 'InverseFlattening',  300.8017,      'Everest 1830'; ...
            ...
            7012, 'clarke80',      'SemimajorAxis', 6378249.145, 'InverseFlattening',  293.465,       'Clarke 1880'; ...
            7022, 'international', 'SemimajorAxis', 6378388,     'InverseFlattening',  297.0,         'International 1924'; ...
            7024, 'krasovsky',     'SemimajorAxis', 6378245,     'InverseFlattening',  298.3,         'Krasovsky 1940'; ...
            7043, 'wgs72',         'SemimajorAxis', 6378135,     'InverseFlattening',  298.26,        'World Geodetic System 1972'; ...
            ...
            7030, 'earth',         'SemimajorAxis', 6378137,     'InverseFlattening',  298.257223563, 'World Geodetic System 1984'; ...
    ...
    methods (Access = private)
        ...
        function self = epsgEllipsoid(self, code)
            ...
            rec = map.internal.epsgread('ellipsoid',code);
            if isempty(rec)
                error(message('map:geodesy:epsgCodeNotFound',code,'ellipsoid'))
            end
            
            % Set the Name and SemimajorAxis properties.
            self.Name = rec.ellipsoid_name;
            self.SemimajorAxis = str2double(rec.semi_major_axis);
            ...
            if ~isempty(rec.inv_flattening)
                self.InverseFlattening = str2double(rec.inv_flattening);
            else
                self.SemiminorAxis = str2double(rec.semi_minor_axis);
            end
            
            % Look up the "unit of measure" code and convert it to
            % a standard value.
            uom_code = str2double(rec.uom_code);
            uom = map.internal.epsgread('unit_of_measure',uom_code);
            self.pLengthUnit = validateLengthUnit(uom.unit_of_meas_name);
        end
